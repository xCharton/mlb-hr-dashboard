...
def fetch_savant_fb(season: int) -> pd.DataFrame:
    """
    Savant custom leaderboard with batted ball breakdown.
    fly_ball column = pure fly ball rate, typically 25-45%.
    """
    url = (
        f"https://baseballsavant.mlb.com/statcast_search/csv"
        f"?all=true&type=details&player_type=batter"
        f"&season={season}&game_type=R&min_pa=50"
        f"&group_by=name&sort_col=pitches&player_event_sort=api_p_release_speed"
        f"&sort_order=desc&stat=fly_ball_rate&csv=true"
    )
    try:
        r = requests.get(url, timeout=20, headers={"User-Agent": "Mozilla/5.0"})
        r.raise_for_status()
    ...
    # Look for any fly ball related column
    fb_col = next(
        (c for c in df.columns if c.lower() in ("fly_ball", "fb", "fly_ball_rate", "fb_rate", "fb_pct", "fly_ball_pct")),
        next((c for c in df.columns if "fly" in c.lower() and "ball" in c.lower()), None)
    )
    if not fb_col:
        st.info(f"FB% cols available: {list(df.columns[:25])}")
        return pd.DataFrame()
