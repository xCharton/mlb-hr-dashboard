...
@st.cache_data(ttl=3600)
def fetch_pitch_mix_by_hand(season: int) -> pd.DataFrame:
    """
    Pulls pitch usage % for all pitchers broken down by batter handedness (L/R).
    Returns one row per pitcher per pitch type per batter hand.
    Savant batSide param: L = vs lefties, R = vs righties.
    """
    frames = []
    for hand in ["L", "R"]:
        url = (
        ...
        frames.append(df[["Pitcher", "batter_hand", pitch_col, usage_col]].rename(
            columns={pitch_col: "pitch_type", usage_col: "usage_pct"}
        ))
...
def build_pitch_mix_str(pitch_mix_by_hand: pd.DataFrame, pitcher: str, batter_hand: str):
    ...
    sub = pitch_mix_by_hand[
        (pitch_mix_by_hand["Pitcher"] == pitcher) &
        (pitch_mix_by_hand["batter_hand"] == batter_hand)
    ].sort_values("usage_pct", ascending=False).head(5)
    if sub.empty:
        return "Pitch mix unavailable"
    return ", ".join(
        f"{row['pitch_type']} {row['usage_pct']:.0f}%"
...
def _la_color(val):
    ...
    if 25 <= v <= 35:                       return "background-color:#2e7d32;color:white"
    elif 15 <= v < 25 or 35 < v <= 45:     return "background-color:#f57f17;color:white"
    else:                                   return "background-color:#e53935;color:white"

def _barrel_color(val):
    try: v = float(val)
    except (TypeError, ValueError): return ""
    if v > 15:      return "background-color:#2e7d32;color:white"
    elif v >= 10:   return "background-color:#388e3c;color:white"
    elif v >= 6:    return "background-color:#f57f17;color:white"
    else:           return "background-color:#e53935;color:white"

def _ev_color(val):
    try: v = float(val)
    except (TypeError, ValueError): return ""
    if v >= 100:    return "background-color:#2e7d32;color:white"
    elif v >= 95:   return "background-color:#388e3c;color:white"
    elif v >= 90:   return "background-color:#f57f17;color:white"
    else:           return "background-color:#e53935;color:white"

def _hh_color(val):
    try: v = float(val)
    except (TypeError, ValueError): return ""
    if v > 50:      return "background-color:#2e7d32;color:white"
    elif v >= 45:   return "background-color:#388e3c;color:white"
    elif v >= 40:   return "background-color:#f57f17;color:white"
    else:           return "background-color:#e53935;color:white"

def _iso_color(val):
    try: v = float(val)
    except (TypeError, ValueError): return ""
    if v > 0.300:   return "background-color:#2e7d32;color:white"
    elif v > 0.250: return "background-color:#388e3c;color:white"
    elif v > 0.200: return "background-color:#f57f17;color:white"
    elif v > 0.160: return "background-color:#e53935;color:white"
    else:           return "background-color:#b71c1c;color:white"

def _woba_color(val):
    try: v = float(val)
    except (TypeError, ValueError): return ""
    if v > 0.400:               return "background-color:#2e7d32;color:white"
    elif 0.370 <= v <= 0.400:   return "background-color:#388e3c;color:white"
    elif 0.340 <= v < 0.370:    return "background-color:#f57f17;color:white"
    else:                       return "background-color:#e53935;color:white"
